...
my-helper connects service providers with people who need their services, capturing leads and service requests with a simple paid tier for provider access.
...
- **Commit as your GitHub identity, or Vercel will block the deploy.** Vercel verifies that
  ...
  ```
  git config user.email "238494190+metab-a11y@users.noreply.github.com"
  git config user.name "metab-a11y"
  ```
